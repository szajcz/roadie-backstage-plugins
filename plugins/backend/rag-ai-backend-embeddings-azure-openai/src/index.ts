...
import {
  AzureOpenAiConfig,
  RoadieAzureOpenAiAugmenter,
} from './RoadieAzureOpenAiAugmenter';
...
export async function initializeAzureOpenAiEmbeddings({
  logger,
  tokenManager,
  vectorStore,
  catalogApi,
  ...
  logger.info('Initializing Roadie Azure OpenAI Embeddings');
  const azureOpenAiConfig = config.get<AzureOpenAiConfig>(
    'ai.embeddings.azureOpenAi',
  );
  ...
  return new RoadieAzureOpenAiAugmenter({
    vectorStore,
    catalogApi,
    discovery,
    augmentationOptions,
